import { WalletConnectionResponse, WalletConnectedWindowMessage } from '../types';
...
function checkForWalletConnection(): boolean {
  try {
...
function setupMessageListener(): void {
  window.addEventListener('message', function(event: MessageEvent) {
    ...
    const data = event.data as WalletConnectedWindowMessage;
    
    if (data.type && data.type === 'WALLET_CONNECTED') {
      console.log('Received wallet connection via postMessage:', data);
