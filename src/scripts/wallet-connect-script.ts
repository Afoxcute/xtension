import { PhantomProvider, SolflareProvider, WalletConnectionResponse } from '../types';

// External script for wallet-connect.html
document.addEventListener('DOMContentLoaded', function() {
  const connectPhantomBtn = document.getElementById('connectPhantom') as HTMLButtonElement;
  const connectSolflareBtn = document.getElementById('connectSolflare') as HTMLButtonElement;
  const statusMessage = document.getElementById('statusMessage') as HTMLDivElement;
  const walletAddressDisplay = document.getElementById('walletAddress') as HTMLDivElement;
  
  // Check if wallets are available
  const phantomAvailable: boolean = !!(window.phantom?.solana?.isPhantom);
  const solflareAvailable: boolean = !!window.solflare;
  ...
  async function connectWallet(provider: PhantomProvider | SolflareProvider): Promise<string | null> {
    try {
      ...
      if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.id) {
        try {
          ...
          chrome.runtime.sendMessage({
            ...
            publicKey: publicKey
          }, function(response: WalletConnectionResponse) {
      ...
      showStatus((error as Error).message || 'Failed to connect wallet', true);
      return null;
    }
  }
  ...
  if (!isSecureContext && !isLocalhost) {
    ...
    if (walletButtons) {
      (walletButtons as HTMLElement).style.opacity = '0.5';
    }
  }
  
  // Set up event listeners for wallet account changes
  if (phantomAvailable && window.phantom?.solana) {
