import { WalletConnectionMessage, WalletConnectionResponse, ExtensionStorageData } from '../types';
...
const GITHUB_PAGES_URL: string = 'https://hoepeyemi.github.io/xtension/wallet-connect.html';

// Local development URLs
const LOCAL_URLS: string[] = [
  'http://localhost:8000/wallet-connect.html',
  'https://localhost:8443/wallet-connect.html'
];

// Function to handle wallet connection
function handleWalletConnection(publicKey: string, sendResponse?: (response: WalletConnectionResponse) => void): void {
...
chrome.runtime.onMessage.addListener(function(
  request: WalletConnectionMessage, 
  sender: chrome.runtime.MessageSender, 
  sendResponse: (response: any) => void
) {
  if (request.action === 'checkWalletConnection') {
    // Check if wallet is connected
    chrome.storage.local.get(['walletConnected', 'walletPublicKey'], function(result: ExtensionStorageData) {
      sendResponse({
    ...
    chrome.storage.local.get(['useLocalDevelopment'], function(result: ExtensionStorageData) {
